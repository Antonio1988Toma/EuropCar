...
<body>
	...
	<div class="container">
		<div align="center">
			<h2>Gestione Aliquota IVA</h2>
			<br>
			<spring:url value="/aliquotaIva/add" var="addUrl"  />
			...
			<c:choose>
				<c:when test="${list.size()>0 }">
					<table class="table table-striped table-bordered mydatatable" style="width: 100%">
						...
						<tbody>
							<c:forEach items="${list}" var="currentAliquotaIva">
								<tr>
									<td>${currentAliquotaIva.aliquota}</td>
									<td>${currentAliquotaIva.descrizione}</td>
									<td>
										<spring:url value="/aliquotaIva/update/${currentAliquotaIva.id }" var="updateUrl"  />
										<a class="btn btn-warning" href="${updateUrl}" role="button">Modifica</a>
									</td>
									<td>
										<spring:url value="/area/delete/${currentAliquotaIva.id }" var="deleteUrl"  />
										<a class="btn btn-danger" href="${deleteUrl}" role="button" onclick="return confirm('Eliminare l\'Aliquota IVA selezionata?')">Elimina</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
