...
<body>
	...
	<div class="container">
		<div align="center">
			<h2>Gestione Area Geografica</h2>
			<br>
			<spring:url value="/area-geo/add" var="addUrl"  />
			...
			<c:choose>
				<c:when test="${list.size()>0 }">
					<table class="table table-striped table-bordered mydatatable" style="width: 100%">
						...
						<tbody>
							<c:forEach items="${list}" var="currentArea">
								<tr>
									<td>${currentArea.codice}</td>
									<td>${currentArea.nomeAreaGeo}</td>
									<td>
										<spring:url value="/area-geo/update/${currentArea.id }" var="updateUrl"  />
										<a class="btn btn-warning" href="${updateUrl}" role="button">Modifica</a>
									</td>
									<td>
										<spring:url value="/area-geo/delete/${currentArea.id }" var="deleteUrl"  />
										<a class="btn btn-danger" href="${deleteUrl}" role="button" onclick="return confirm('Eliminare l\'Area Geografica selezionata?')">Elimina</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
