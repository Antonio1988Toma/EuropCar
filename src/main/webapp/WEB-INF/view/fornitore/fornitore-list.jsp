			...
			<c:choose>
				<c:when test="${list.size()>0 }">
					<table class="table table-striped table-bordered mydatatable" style="width: 100%">
						...
						<tbody>
							<c:forEach items="${list}" var="currentFornitore">
								<tr>
									...
									<td>
										<spring:url value="/fornitore/update/${currentFornitore.id }" var="updateUrl"  />
										<a class="btn btn-warning" href="${updateUrl}" role="button">Modifica</a>
									</td>
									<td>
										<spring:url value="/fornitore/delete/${currentFornitore.id }" var="deleteUrl"  />
										<a class="btn btn-danger" href="${deleteUrl}" role="button" onclick="return confirm('Eliminare il Fornitore selezionata?')">Elimina</a>
									</td>
								</tr>
							</c:forEach>
						</tbody>
